...
## Phase 2: Validation Hook Implementation [checkpoint: 7bd1142]
...
- [x] Task: Conductor - User Manual Verification 'Phase 2: Validation Hook' (Protocol in workflow.md)

## Phase 3: Function Integration
- [x] Task: Integrate validation hook into `motis_plan()` and `motis_plan_txt_1()`.
- [x] Task: Integrate validation hook into `motis_one_to_many()` and its batch/intermodal variants.
- [x] Task: Integrate validation hook into `motis_one_to_all()`.
